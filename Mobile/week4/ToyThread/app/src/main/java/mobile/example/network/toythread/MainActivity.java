...
//                쓰레드 생성 후 실행
            ...
            case R.id.button2:
                ...
                //우리가 고의로 발생시킨 예외상황 -> 쓰레드 중단되면서 현재의 결과가 출력
                //쓰레드가 null일 가능성이 있기 때문
                //즉 null이 아니면 쓰레드를 중단시킴
                break;
            case R.id.button3:
                Toast.makeText(this, "Toast!", Toast.LENGTH_SHORT).show();
                //쓰레드 동작 상태에서 별도의 상태에서 토스트 동작
                break;
        }
    }
...
//    쓰레드 클래스
    class ToyThread extends Thread {
        ...
        public ToyThread(Handler handler, String data) {
            //handler를 전달받았다 = 호출한 쪽에다 메세지를 전달하겠다
            //별도의 데이터도 받음
            this.data = data;
...
//            Message 에 실행 결과 저장
            ...
            //DTO를 전달하고 싶다면 해당 객체를 Serializable하면 됨
            //즉 Message.obj에는 serializable 인터페이스를 구현한 객체만 저장 가능
